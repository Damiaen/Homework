...
namespace GevorderdProgrammerenPracticumWeek1
{
	...
	public partial class MainWindow : Window
	{
		...
		{
			int leeftijd;
			if (VoornaamTextBox.Text != "" && AchternaamTextBox.Text != "" && int.TryParse(LeeftijdTextBox.Text, out leeftijd))
			{
				personen.Add(new Persoon(VoornaamTextBox.Text, AchternaamTextBox.Text, leeftijd));
				showPersonen();
		...
		private void AddHuisdierButton_Click(object sender, EventArgs e)
		{
			...
			{
				selectedPersoon?.AddHuisdier(new Huisdier(HuisdierNaamTextBox.Text, HuisdierSoortTextBox.Text));
